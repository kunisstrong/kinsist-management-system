...
const addDialogBtnDisabled = computed(() => {
  const res: boolean =
    addFormParams.value.deptName !== "" && addFormParams.value.manager !== "" && addFormParams.value.allNum !== "";
  return !res;
});
/* 清空新增参数 */
...
const updateDialogBtnDisabled = computed(() => {
  const res: boolean =
    updateParams.value.deptName !== "" && updateParams.value.manager !== "" && updateParams.value.allNum !== "";
  return !res;
});
