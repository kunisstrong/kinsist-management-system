  ...
  <div class="staff-container">
    ...
    <el-dialog v-model="updateFormVisible" title="员工信息修改">
      ...
      <template #footer>
                <span class="dialog-footer">
                    <el-button @click="addFormVisible = false">取消</el-button>
                    <el-button type="primary" @click="postUpdateCheckProject"
                               :disabled="updateBtnDisabled">确认</el-button>
                </span>
      </template>
    </el-dialog>
    <div class="table">
      <el-table ref="multipleTableRef" :data="tableData" max-height="76vh" border style="width: 100%" @select="select"
        ...
        <el-table-column property="salary" label="薪水(千)" align="center"/>
...
const postUpdateCheckProject = async () => {
  ...
  // tableData.value[tableData.value.indexOf(updateFormInfo.value)] = updateFormInfo.value
  tableData.value.forEach((item: staffManagementTable) => {
    if (item.id === updateFormInfo.value.id) {
      item.name = updateFormInfo.value.name
      item.sex = updateFormInfo.value.sex
      item.postion = updateFormInfo.value.postion
      item.age = updateFormInfo.value.age
      item.entryData = updateFormInfo.value.entryData
      item.salary = updateFormInfo.value.salary
    }
  })
  ElMessage({
    message: '修改成功',
